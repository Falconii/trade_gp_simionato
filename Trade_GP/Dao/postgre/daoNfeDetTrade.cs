...
namespace Trade_GP.Dao.postgre
{
    class daoNfeDetTrade
    {
        ...
        public void Delete(NfeDetTrade obj)
        {
            ...
            try
            {
                String StringDelete = $" DELETE FROM  NFE_DET_TRADE  WHERE ID_GRUPO = {obj.Id_Grupo} AND AND ID_PLANILHA = {obj.Id_Planilha} AND NRO_LINHA = {obj.Nro_Linha} ";
        ...
        public void DeleteByIdPlanilha(int id_Grupo, int id_Planilha)
        {
            ...
            try
            {
                String StringDelete = $" DELETE FROM  NFE_DET_TRADE  WHERE ID_GRUPO = {id_Grupo} AND ID_PLANILHA = {id_Planilha} ";
        ...
        public NfeDetTrade Insert(NfeDetTrade obj)
        {
            /*
        ...
        public void Update(NfeDetTrade obj)
        {
            /*
        ...
        public int ClearBonificacao(int grupo, int planilha, int linha)
        {
            int retorno = 0;

            String StringUpdate = $" UPDATE nfe_det_trade_5910 SET " +
             "   VENDA_STATUS    = 'NA' " +
            $"   WHERE ID_GRUPO  =  {grupo} AND ID_PLANILHA = {planilha} AND NRO_LINHA = {linha}  RETURNING *";
        ...
        public async Task<int> Processou_Devolucao(int id_grupo, string cod_emp, string local, string periodo)
        {
            ...
            String StringProc = $"SELECT COUNT(*) AS TOTAL FROM nfe_det_trade dev where dev.id_grupo = '{id_grupo}' and dev.cod_emp = '{cod_emp}' and dev.local = '{local}' and dev.id_operacao = 'Z' and to_char(dev.dt_ref,'MM/YYYY') = '{periodo}' and dev.dt_ref >= '2017-03-16' AND dev.id_saida = 0;";
        ...
        public BoniNota SeekByBoniNota(int id_grupo, string cod_emp, string cnpj_cpf, string material, string nro_doc)
        {
            ...
            string strSelect = "select ven.id_grupo,ven.id_planilha,ven.nro_linha,ven.qtd_convertida,boni_planilha,boni_linha " +
                ...
                using (var objCommand = new NpgsqlCommand(strSelect, objConexao))
                {
                    try
                    {
                        ...
                        if (objDataReader.HasRows)
                        {
                            ...
                            obj = PopulaNfBoni(objDataReader);
        ...
        public BoniNota SeekByBoniData(int id_grupo, string cod_emp, string cnpj_cpf, string material, DateTime dt_ref)
        {
            ...
            string strSelect = "select ven.id_grupo,ven.id_planilha,ven.nro_linha,ven.qtd_convertida,boni_planilha,boni_linha " +
                              ...
                              $"   and ven.cnpj_cpf = '{cnpj_cpf}' " +
                              $"   and ven.material = '{material}' AND boni_planilha = 0 ";
        ...
        private BoniNota PopulaNfBoni(NpgsqlDataReader objDataReader)
        {
            ...
            var obj = new BoniNota();
            obj.Id_Grupo = Convert.ToInt32(objDataReader["id_grupo"].ToString());
            obj.Id_Planilha = Convert.ToInt32(objDataReader["id_planilha"].ToString());
            obj.Nro_Linha = Convert.ToInt32(objDataReader["nro_linha"].ToString());
            obj.Qtd_Convertida = Convert.ToDouble(objDataReader["qtd_convertida"]);
            obj.Boni_Planilha = Convert.ToInt32(objDataReader["boni_planilha"].ToString());
...
public NfeDetTrade Seek(int id_grupo, int id, int nrolinha)
{

    NfeDetTrade obj = null;
...
public NfeDetTrade SeekNfeConsulta(int id, int nrolinha)
{
...
private NfeDetTrade PopulaNfeDet(NpgsqlDataReader objDataReader)
{

    var obj = new NfeDetTrade();
    obj.Id_Grupo = Convert.ToInt32(objDataReader["id_grupo"].ToString());
...
private NfeDetTrade PopulaNfeDetFull(NpgsqlDataReader objDataReader)
{
